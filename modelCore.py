...
class Conv2DSymPadding( _Conv ) :
    ...
    def __init__(self, filters,
        ...
        super(Conv2DSymPadding, self).__init__(
            ...
            **kwargs)#卷基层参数初始化函数
        self.input_spec = InputSpec(ndim=4)#输入空间维度是4
    def get_config(self):
        config = super(Conv2DSymPadding, self).get_config()
        config.pop('rank')
        return config#输入self配置卷基层参数，返回卷基层的配置
    def call( self, inputs ) :
        if ( isinstance( self.kernel_size, tuple ) ) :#判断输入的卷积核大小是不是元组，元组不可以修改，只能增加新的部分
            kh, kw = self.kernel_size#如果输入的卷积核不能修改，那就记录卷积核大小
        else :
            kh = kw = self.kernel_size#如果卷积核大小可以修改，那就
        ph, pw = kh//2, kw//2
        ...
        outputs = K.conv2d(
                ...
                dilation_rate=self.dilation_rate)#设置输出值
        if self.use_bias:
            outputs = K.bias_add(
                outputs,
                self.bias,
        ...
        if self.activation is not None:#如果激活内容不为空
            return self.activation(outputs)#返回激活后的参数
        return outputs#如果不需要激活，直接返回输出参数
        
class BayarConstraint( Constraint ) :
#定义BayerConv2D,这是一种约束卷积层的方式，可以抑制图像内容对篡改痕迹的影响，自适应提取图像的篡改特征
    def __init__( self ) :
        self.mask = None#定义输入掩码是空？
...
#初始化SRMConv2D隐写分析的模型滤波器，利用SRM层提取的噪声特征来发现真实和篡改区域之间的噪声不一致，三个卷积层使用组合效果最好
        # srm kernel 1                                                                                                                                
    ...
    def _build_SRM_kernel( self ) :#构造SRM卷积核
        kernel = []
        ...
        return srm_kernel
    def build( self, input_shape ) :#定义输入类型
        ...
        # 1. regular conv kernels, fully trainable    常规的卷积核，全训练？                                                                                                
        filters = self.filters - 9 - 3#更新滤波器
        if filters >= 1 :
        ...
        # 2. SRM kernels, not trainable                    不训练SRM的卷积核                                              
        self.srm_kernel = self._build_SRM_kernel()
        # 3. bayar kernels, trainable but under constraint                    选练bayer的卷积核，但是受约束                                                                        
        ...
        # 4. collect all kernels               集合所有的卷积核                                                                                                       
