...
class _deTransition(nn.Sequential):
    def __init__(self, num_input_features: int, num_output_features: int) -> None:
        ...
        self.add_module('conv', torch.nn.ConvTranspose2d(num_input_features, num_output_features, kernel_size=1,
...
class DenseNet(nn.Module):
    def __init__(
        ...
        for i, num_layers in enumerate(block_configdown):
            block = _DenseBlock(
            ...
            if i != len(block_configdown) - 1:
                trans = _Transition(num_input_features=num_features,
                                    num_output_features=int(num_features*compression_rate))
                self.features.add_module('transition%d' % (i + 1), trans)
                num_features = int(num_features * compression_rate)

        # module between downsampling and upsampling
        self.features.add_module('norm5', nn.BatchNorm2d(num_features))
        self.features.add_module('relu5',nn.ReLU(inplace=True))
        self.features.add_module('fullyconv',nn.Conv2d(num_features,2,kernel_size=1,stride=1,bias=False))
        self.features.add_module('concation0',concat)

        #upsampling
        num_features = num_features+2
        for i, num_layers in enumerate(block_configup):
            block = _DenseBlock(
                num_layers=num_layers,
                num_input_features=num_features,
                bn_size=bn_size,
            ...
            if i==0:
                trans = first_Transition(num_input_features=num_features,
                                    num_output_features=int(num_features * compression_rate))
                self.features.add_module('detransition%d' % (i + 1), trans)
                num_features = int(num_features * compression_rate)

            self.features.add_module('updenseblock%d' % (i + 1), block)
            num_features = num_features + num_layers * growth_rate

            if i != ((len(block_configup) - 1) | 0) :
                trans = _deTransition(num_input_features=num_features,
                                    num_output_features=int(num_features*compression_rate))
                self.features.add_module('detransition%d' % (i + 1), trans)
                num_features = int(num_features * compression_rate)
...
def _densenet(
    arch: str,
    growth_rate: int,
    block_configdown: Tuple[int, int, int, int],
    num_init_features: int,
    pretrained: bool,
    progress: bool,
    **kwargs: Any
) -> DenseNet:
    model = DenseNet(growth_rate, block_configdown, num_init_features, **kwargs)
    if pretrained:
